...
def create_model(captcha_length, captcha_num_symbols, input_shape, model_depth=5, module_size=2):
  ...
  for i, module_length in enumerate([module_size] * model_depth):
      for j in range(module_length):
          x1 = keras.layers.Conv2D(32*2**min(i, 3), kernel_size=3, padding='same', kernel_initializer='he_uniform')(x1)
          x1 = keras.layers.BatchNormalization()(x1)
          x1 = keras.layers.Activation('relu')(x1)
      x1 = keras.layers.MaxPooling2D(2)(x1)

  x1 = keras.layers.Flatten()(x1)
  x1 = [keras.layers.Dense(captcha_num_symbols, activation='softmax', name='char_%d'%(i+1))(x1) for i in range(captcha_length)]
  model = keras.Model(inputs=input_tensor, outputs=x1)
...
class ImageSequence(keras.utils.Sequence):
    def __init__(self, directory_name, batch_size, captcha_length, captcha_symbols, captcha_width, captcha_height):
        ...
        self.files = dict(zip(map(lambda x1: x1.split('.')[0], file_list), file_list))
        self.used_files = []
    ...
    def __getitem__(self, idx):
        ...
        for i in range(self.batch_size):
            random_image_label_1 = random.choice(list(self.files.keys()))
            random_image_file = self.files[random_image_label_1]

            # We've used this image now, so we can't repeat it in this iteration
            self.used_files.append(self.files.pop(random_image_label_1))
            ...
            random_image_label_1 = random_image_label_1.split('_')[0]

            for j, ch in enumerate(random_image_label_1):
                y[j][i, :] = 0
...
def main():
    ...
    #with tf.device('/device:XLA_CPU:0'):
        ...
        training_data_1 = ImageSequence(args.train_dataset, args.batch_size, args.length, captcha_symbols, args.width, args.height)
        validation_data_1 = ImageSequence(args.validate_dataset, args.batch_size, args.length, captcha_symbols, args.width, args.height)
        ...
        try:
            model.fit_generator(generator=training_data_1,
                                validation_data_1=validation_data_1,
                                epochs=args.epochs,
                                callbacks=callbacks,
